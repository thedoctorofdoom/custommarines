...
2. **Never reference a PB-owned CVar as a bare ZScript identifier** — UZDoom’s ZScript compiler resolves bare identifiers against the live CVar list at compile time (`FxIdentifier::Resolve` -> `FindCVar`); if PB_Staging has removed that CVar (as happened with `pb_keepweapons`, `pb_grenadeimpact`, and `pb_grenadeshrapnel`), this is a fatal **"Unknown identifier"** compile error that blocks the entire mod from loading. Always go through the null-safe helpers in [`PBCM_CVarCompat`](zscript/PBCM_CVarCompat.zc) (`GetIntOr`/`GetBoolOr`/etc., e.g. `PBCM_CVarCompat.GetBoolOr('pb_keepweapons')`) for any PB-owned CVar accessed as an identifier. Do **not** "fix" a missing PB CVar by redeclaring its name in this mod’s own `CVARINFO` — UZDoom’s `ParseCVarInfo` hard-errors with `cvar '%s' already exists` if any already-loaded file (including an older PB copy some users might still have) defines the same name, which just trades one load-blocking crash for another.

3. **Optional cleanups** (warnings, not always load-blocking): replace deprecated flags and old member names where the engine warns; align railgun puff / `A_CustomRailgun` actor names with PB_Staging if PB renames puffs; avoid passing `null` where `class<PB_WeaponBase>` is resolved strictly—prefer the concrete weapon class string for that marine file.
